...
setwd("C:/Users/pluto/Desktop/gender-recognition-by-voice")
library(devtools)
library(parallel)
...
specan3 <- function(X, bp = c(0,22), wl = 2048, threshold = 5, parallel = 1){
  ...
  if(parallel > 1)
  { options(warn = -1)
    if(all(Sys.info()[1] == "Linux",requireNamespace("parallelsugar", quietly = TRUE) == TRUE)) 
      lapp <- function(X, FUN) parallelsugar::mclapply(X, FUN, mc.cores = parallel) else
        if(Sys.info()[1] == "Linux"){ 
          cat("Linux users need to install the 'parallelsugar' package for parallel computing (you are not doing it now!)")
...
write.csv(data, file='voice1.csv',row.names=F)
